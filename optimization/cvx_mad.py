...
c = [0.0 for _ in range(N)] + [1.0/K for _ in range(K)]

A = [1.0 for _ in range(N)] + [0.0 for _ in range(K)]
b = [1]
h = [0.0 for _ in range(2*(N+K))]
G_y = []
...
for sign in [1, -1]:
    for k in range(K):
        row = []
        for i in range(N):
            row.append(means[i]-returns[i][k])
        G_x.append(map(float, sign*np.array(row)))

G = map(list.__add__, G_x, G_y)
print "c", np.array(c).shape
print "G", np.array(G).shape
print "h", np.array(h).shape
